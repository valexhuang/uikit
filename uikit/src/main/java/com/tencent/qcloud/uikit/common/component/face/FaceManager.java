...
public class FaceManager {
    ...
    private static Context context = ILiveUIKit.getAppContext();
    ...
    public static void loadEmojiFiles() {
        try {
            String files[] = context.getAssets().list("emoji");
            for (int i = 0; i < files.length; i++) {
                loadAssetBitmap(emojiFilters[i], "emoji/" + files[i]);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static Bitmap loadAssetBitmap(String filter, String assetPath) {
        InputStream is = null;
        try {
